...
from qqsms import sendMsgToAdmin
...
def get_list_from_http_lt_yyt(sect_code):
    phone_sect_list=[]
    url = "http://num.10010.com/NumApp/NumberCenter/qryNum?callback=jsonp_queryMoreNums&provinceCode=11&cityCode=110&advancePayLower=0&sortType=1&goodsNet=4&searchCategory=3&qryType=01&numNet=%s&groupKey= " %(sect_code)
...
def get_score(phone):
	...
	print "hexingshuli",hexingshuli
        
        quantishuli = bref_lst[1].find('font').get_text()  
	print "quantishuli",quantishuli
	    ...
	    print "shuzinengliang_num",shuzinengliang_num
            break
...
def loop2(sect):
    ...
    #phone_sect_list= get_list_from_file("temp_score90.txt")
    file="/root/test/bjlt/f_lt_out_%s.txt" %(sect)
    f = open(file,'a')
    #head="phone total_score,核心数理,全体数理,易经长卦,易经短卦,数字能量吉数,五行\n"
    #f.write(head)
    for phone in phone_sect_list:
	phone  ="18600706272"
        score,hexingshuli,quantishuli,yjchanggua,yjduangua,shuzinengliang_num,wuxing = get_score(phone)
        print  score,hexingshuli,quantishuli,yjchanggua,yjduangua,shuzinengliang_num,wuxing
        print "aaaaaaaaaaaaaaaaaaaaaa"
        ss = "%s %s %s %s %s %s %s %s \n" % (phone,score,hexingshuli,quantishuli,yjchanggua,yjduangua,shuzinengliang_num,wuxing )
        ss = ss.encode("utf8")
        
        shuzi_num= int(shuzinengliang_num.replace(u"吉",""))
        if int(score) >= 90 and wuxing==u"金" and shuzi_num >= 9:
            sendMsgToAdmin("选号喽",ss)
        if int(score) >= 80 and shuzi_num >= 8:        
            f.write(ss)
            #print ss
