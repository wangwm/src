...
def loop2(sect):
    ...
    #file="/root/test/bjlt/lt_%s.txt" %(sect)
    file="/root/test/bjlt/lt_%s.txt" %("all")
    f = open(file,'a')
	...
	sec = phone[:3]
	left = phone[3:] 
	if sec == "186" or sec == "185" or  sec == "156" or  sec == "155" or sec == "166":
		res = re.findall("[1,3,4,9]",left)
		if len(res) <=0:
			f.write(phone+"\n")
			print phone
	elif  sec == "131" or sec == "130":
		res = re.findall("[2,5,6,7,8]",left)
		if len(res) <=0:
			f.write(phone+"\n")
			print phone
	else:
